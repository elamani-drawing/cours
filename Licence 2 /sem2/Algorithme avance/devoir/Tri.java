...
public class Tri implements Sort<Integer> {
    ...
    /**
     *
     * @param n le nombre d'element de la liste
     * @param m le valeur maximal que peux prendre un element
     */
     ...
     */
    public SList<Integer> triDecroissantInsertion() {
        Node p = this.head;
        if (p == null) {
            return new SList<Integer>(); //si cest vide on retourne la liste vide
        }
        this.headSorted=null;//on réenitialise la liste ranger au cas ou un autre tri a ete realiser
     ...
     * @param elem est l'element à inseret
     */
    private void sortedDecroissantInsert(Node<Integer> elem) {
        //si on insert en debut de liste
        if (headSorted == null) {
            ...
            //on cherche la position i ou on doit inseret elem (on sarrete a i)
            while (p != null && p.value >= elem.value) {
                tmp = p;
                p = p.next;
            }
            //on recolle les  nodes
            if (tmp == null) {
                //lelement se met en debut de liste
                elem.next = p;
                headSorted = elem;
                this.headSorted=headSorted;
            } else {
